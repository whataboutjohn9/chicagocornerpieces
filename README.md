# Chicago Corner Pieces - v1, Daily Chicago Trivia

## Project info
# 🍕 Chicago Corner Pieces
### *Part 1 - The Daily Chicago Trivia Trail*

**The Chicago Trail** is a daily trivia game that mashes up the high-stakes survival of *The Oregon Trail* with the gritty, hyper-local knowledge of Chicago. Navigate the city's neighborhoods, avoid "pothole dysentery," and see if you can make it to the lakefront without running out of giardiniera.

**Play the game here:** [https://whataboutjohn.com/apps/trivia/](https://whataboutjohn.com/apps/trivia/)

---

## 🕹️ Gameplay
* **Daily Challenge:** A fresh set of Chicago-centric questions every day.
* **Survival Mechanics:** Your progress through the city depends on your trivia accuracy. Wrong answers might cost you "health" or delay your arrival.
* **Local Lore:** Questions cover everything from political scandals and sports history to the exact location of the best "hidden gem" beef stand.

## 🛠️ Technical Stack
This project is built with a modern frontend stack:
* **Framework:** [React](https://react.dev/) + [Vite](https://vitejs.dev/)
* **Language:** [TypeScript](https://www.typescriptlang.org/)
* **Styling:** [Tailwind CSS](https://tailwindcss.com/) + [shadcn/ui](https://ui.shadcn.com/)
* **Deployment:** [Lovable](https://lovable.dev/)

## 🚀 Getting Started
If you want to run the "trail" locally:

1. **Clone the repo:**
   ```bash
   git clone [https://github.com/whataboutjohn9/chicagocornerpieces.git](https://github.com/whataboutjohn9/chicagocornerpieces.git)

2. **Install dependencies:**

'''bash
    npm install

## 🏗️ Contribution
Have a trivia question that’s so "Chicago" it hurts? Feel free to open an issue or submit a pull request. Just remember: No Ketchup on the codebase.

